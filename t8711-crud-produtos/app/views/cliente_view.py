...
class Cliente_View:
    ...
    def configurar_treeview(self):
        ...
        self.tbl_clientes.column("#0", width=0, stretch=False)
        self.tbl_clientes.column("id", width=10, anchor="center")
        self.tbl_clientes.column("nome", width=40)
        self.tbl_clientes.column("nascimento", width=20, anchor="center")
        self.tbl_clientes.column("idade", width=15, anchor="center")
        self.tbl_clientes.column("limite", width=20, anchor="e")
        self.tbl_clientes.column("cidade", width=30)
        self.tbl_clientes.column("estado", width=15, anchor="center")

        self.tbl_clientes.heading("id", text=Idioma.t("comum.id"))
        self.tbl_clientes.heading("nome", text=Idioma.t("comum.nome"))
        self.tbl_clientes.heading("nascimento", text=Idioma.t("cliente.coluna_nascimento"))
        self.tbl_clientes.heading("idade", text=Idioma.t("cliente.coluna_idade"))
        self.tbl_clientes.heading("limite", text=Idioma.t("cliente.coluna_limite"))
        self.tbl_clientes.heading("cidade", text=Idioma.t("cidade.estado"))
        self.tbl_clientes.heading("estado", text=Idioma.t("cliente.coluna_uf"))
    ...
    def configurar_eventos(self):
        ...
        self.tbl_clientes.bind("<<TreeviewSelect>>", self.controller.selecionar_cliente)
        self.cmb_estados.bind("<<ComboboxSelected>>", self.controller.carregar_cidades_do_estado_selecionado)

    def carregar_estados(self, estados):
        self._estados = estados
        valores = [f"{estado.id} - {estado.nome}" for estado in estados]
        self.cmb_estados["values"] = valores
        self.cmb_estados.set("")

    def carregar_cidades(self, cidades):
        self._cidades = cidades
        valores = [f"{cidade.id} - {cidade.nome}" for cidade in cidades]
    ...
    def preencher_campos(self, cliente, cidades):
        ...
        self.txt_id.insert(0, str(cliente.id))
        self.txt_id.config(state="readonly")

        self.txt_nome.insert(0, cliente.nome)
        self.txt_data_nascimento.insert(0, Data_Utils.data_para_string(cliente.data_nascimento))
        self.txt_limite_credito.insert(0, str(cliente.limite_credito))
    ...
    def ler_dados_cliente(self):
        ...
        if not Data_Utils.validar_data(data_nascimento):
            raise ValueError(Idioma.t("cliente.erro_data_invalida"))

        try:
            limite_credito = float(self.txt_limite_credito.get())
        except ValueError:
            raise ValueError(Idioma.t("cliente.erro_limite_invalido"))

        indice = self.cmb_cidades.current()
        if indice < 0:
            raise ValueError(Idioma.t("cliente.erro_selecionar_cidade"))

        cidade = self._cidades[indice]
        return nome, data_nascimento, limite_credito, cidade

    def exibir_mensagem(self, mensagem, sucesso=True):
        titulo = Idioma.t("cidade.sucesso")
        if sucesso:
