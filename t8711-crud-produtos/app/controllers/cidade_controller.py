    ...
    def update(self):
        try:
            if self.cidade_selecionada is None:
                self.view.exibir_mensagem (idioma.t("cidade.selecionar"), False)
                return
            nome, estado = self.view.ler_dados_cidade()
            self.cidade_selecionada.atualizar_dados(nome, estado)
            self.dao.update(self.cidade_selecionada)
            self.get_all()
            self.view.exibir_mensagem (Idioma.t("cidade.excluida_sucesso"))
        except ValueError as e:
            self.view.exibir_mensagem (f"{Idioma.t('comum.erro_prefixo')}{Idioma.t(str(e))}", False)
    ...
    def delete(self):
        ...
        try:
            ...
            if sucesso:
                ...
                self.view.exibir_mensagem(Idioma.t("cidade.nao_encontrada"), False)
        except Exception as e:
            self.view.exibir_mensagem(Idioma.t("cidade.problemas"), False)
