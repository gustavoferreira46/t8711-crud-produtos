...
class Categoria_Controller:
    ...
    def save(self):
        try:
            ...
            self.get_all()
            self.view.exibir_mensagem(Idioma.t("categoria.cadastrada_sucesso"))
    ...
    def update(self):
        try:
            if self.categoria_selecionada is None:
                self.view.exibir_mensagem(Idioma.t("selecionar_categoria"), False)
                return
            nome = self.view.ler_dados_categoria()
            self.categoria_selecionada.atualizar_dados(nome)
            self.dao.update(self.categoria_selecionada)
            self.get_all()
            self.view.exibir_mensagem(Idioma.t("categoria.atualizada_sucesso"))
        except ValueError as e:
            self.view.exibir_mensagem(f"{Idioma.t('comum.erro_prefixo')}{Idioma.t(str(e))}", False)
    ...
    def delete(self):
        if self.categoria_selecionada is None:
            self.view.exibir_mensagem(Idioma.t("categoria.excluida_sucesso"), False)
            return
        ...
        try:
            ...
            if sucesso:
                ...
                self.view.limpar_campos()
                self.get_all()
                self.view.exibir_mensagem(Idioma.t("categoria.excluida.sucesso"))
            else:
                self.view.exibir_mensagem(Idioma.t("categoria.nao_encontrada"), False)
        except Exception as e:
            self.view.exibir_mensagem(Idioma.t("categoria.problemas"), False)
